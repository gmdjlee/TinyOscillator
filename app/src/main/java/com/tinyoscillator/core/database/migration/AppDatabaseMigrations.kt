...
/**
 * 모든 Room 스키마 마이그레이션을 한 곳에 모은 모듈.
 *
 * Phase 4.5 분할 이전에는 `DatabaseModule.kt`에 800+ 줄에 걸쳐 마이그레이션·DAO 프로바이더·
 * AppDatabase 빌더·포트폴리오 백업 로직이 섞여 있었다. 파일 하나의 책임을 분명히 하기 위해
 * 마이그레이션은 본 파일, DAO 프로바이더는 `DaoModule.kt`, 빌더는 `DatabaseModule.kt`에 분리.
 *
 * 공개 API는 [ALL] 배열 하나뿐이며 `DatabaseModule.provideAppDatabase`에서 사용한다.
 *
 * 마이그레이션 추가 절차:
 *  1. `MIGRATION_N_M` 객체를 아래 정의 블록에 추가.
 *  2. [ALL]의 `arrayOf(...)` 끝에 해당 객체 추가.
 *  3. `AppDatabase @Database(version = M)` 승격.
 *  4. `schemas/com.tinyoscillator.core.database.AppDatabase/<M>.json` export 확인.
 */
...
/** Migration v24→v25: added analysis_snapshots table */
        ...
        try {
            db.execSQL("""
                CREATE TABLE IF NOT EXISTS `analysis_snapshots` (
                    ...
                    `algo_scores` TEXT NOT NULL,
                    `algo_rationales` TEXT NOT NULL
                )
            """.trimIndent())
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_analysis_snapshots_ticker_analyzed_at` ON `analysis_snapshots` (`ticker`, `analyzed_at`)")
            db.execSQL("CREATE INDEX IF NOT EXISTS `index_analysis_snapshots_ticker` ON `analysis_snapshots` (`ticker`)")
            Timber.d("Migration v24→v25 성공: analysis_snapshots 테이블 생성")
        } catch (e: Exception) {
...
/** Migration v25→v26: added OHLCV columns to analysis_cache */
private val MIGRATION_25_26 = object : Migration(25, 26) {
    override fun migrate(db: SupportSQLiteDatabase) {
        try {
            db.execSQL("ALTER TABLE `analysis_cache` ADD COLUMN `open_price` INTEGER NOT NULL DEFAULT 0")
            db.execSQL("ALTER TABLE `analysis_cache` ADD COLUMN `high_price` INTEGER NOT NULL DEFAULT 0")
            db.execSQL("ALTER TABLE `analysis_cache` ADD COLUMN `low_price` INTEGER NOT NULL DEFAULT 0")
            db.execSQL("ALTER TABLE `analysis_cache` ADD COLUMN `volume` INTEGER NOT NULL DEFAULT 0")
