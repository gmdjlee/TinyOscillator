...
class GlobalIndexRegistryTest {
    ...
    @Test
    fun `자동 커버 지수는 stooqTicker가 채워져 있다`() {
        assertTrue(GlobalIndexRegistry.AUTO_COVERED.all { it.stooqTicker != null })
    }

    @Test
    fun `수동 필요 지수는 stooqTicker가 없다`() {
        val manualSpecs = GlobalIndexRegistry.OVERSEAS_MARKETS.filter { it.name in GlobalIndexRegistry.MANUAL_REQUIRED_NAMES }
        assertTrue(manualSpecs.all { it.stooqTicker == null })
    }
