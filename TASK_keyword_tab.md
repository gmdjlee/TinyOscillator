...
- **Phase 2 완료(2026-07-21)**: UI 컴포넌트 3건. ①`presentation/etf/EtfAnalysisContent.kt` `EtfListItem` `private`→`public` 승격(시그니처 `etf`·`includeKeywords`·`onClick` 유지, `@OptIn(ExperimentalLayoutApi)` 보존) — 키워드 상세 pane과 공유. ②`presentation/keyword/KeywordListContent.kt` 신규(`ThemeListContent` 미러): `CollectionProgressBar(EtfUpdateWorker.TAG)`+검색("키워드 검색")+정렬칩(ETF수/평균등락/키워드명)+카운트·갱신버튼+`KeywordCard`(키워드명·`ETF N개`·우측 `평균 X%` `signColor` 부호색). 빈뷰 2변형(포함키워드 0→"설정에서 필터 키워드 등록"·그외→"ETF 목록 탭에서 수집"). ③`presentation/keyword/KeywordDetailPane.kt` 신규(`ThemeDetailPane` 미러): `group: KeywordGroup?` 직접 수신(별도 VM 불필요 — `KeywordViewModel.groups`가 이미 계산, Phase 3에서 selectedKeyword→group 해소), null=미선택 안내, `HeaderCard`(ETF수+평균등락률 `signColor`)+멤버 `EtfListItem` LazyColumn(→`onEtfClick(ticker)`). 한국 등락색 `signColor`/`LocalFinanceColors`만 사용(하드코딩 0). `:app:assembleDebug` BUILD SUCCESSFUL(app-debug.apk 25.7MB 산출), 회귀 0. Compose는 단위테스트 대상 아님(컴파일 검증). ExploreScreen 배선(Phase 3) 미착수.
- **Phase 3 완료(2026-07-21)**: `presentation/explore/ExploreScreen.kt` 배선. ①`ExploreTab` enum에 `KEYWORD("키워드")` 삽입(통계↔테마 사이) — enum은 `private`, `entries.toList()`가 탭 Row·`when` 분기만 구동하므로 재정렬 안전(기존 브랜치 무수정). ②`selectedKeyword: String?` `rememberSaveable`(기본 saver, listSaver 불필요). ③`KEYWORD` when 분기: `hiltViewModel<KeywordViewModel>()` 1개 인스턴스를 목록·상세 공유(내부 default `hiltViewModel()` 미평가), `groups`/`includeKeywords` `collectAsStateWithLifecycle` → `selectedGroup = groups.find { keyword == selectedKeyword }`. **2-Pane**: `TwoPaneLayout`(list=`KeywordListContent(onKeywordClick=set)`, detail=`KeywordDetailPane(group=selectedGroup, onEtfClick=onEtfDetailClick)` — 결정 A(b) 전체화면 push, group=null→내장 "키워드를 선택해주세요" placeholder). **COMPACT**: 결정 B(a) 로컬 state 토글 — `selectedGroup!=null`이면 `KeywordDetailPane`+`BackHandler{selectedKeyword=null}`(nav 그래프 미변경), 아니면 `KeywordListContent`. import 추가: `BackHandler`(activity-compose 1.8.2)·`hiltViewModel`·`collectAsStateWithLifecycle`·keyword 3파일. `:app:assembleDebug` BUILD SUCCESSFUL(exit 0), 기존 탭(ETF/통계/테마/리포트) 코드 무변경=회귀 0. 실기 QA(Phase 4) 미착수.
- **Phase 4 완료(2026-07-21, 실기 QA)**: 에뮬레이터 pixel_fold(sdk_gphone64, 2076×2152 dp390, display id `4619827259835644672`). `assembleDebug`→install→구동. **수용 전 항목 통과, 크래시 0**(logcat FATAL/AndroidRuntime/Exception 0, MainActivity 세션 내내 동일 task 생존). 검증: ①탭 순서 **ETF·통계·키워드·테마·리포트**(키워드=통계↔테마 사이) ✓ ②키워드 카드 렌더(21그룹, ETF수 내림차순 8→6→5→4→3, `평균 X%` signColor −청/+적) ✓ ③카드→상세: 바이오 헤더(ETF 8개·평균 -1.48%)+멤버 `EtfListItem`(배지·티커·부호색) ✓ ④**중복 매칭 실증**: KoAct 바이오헬스케어=[바이오][헬스케어] 양 그룹 등장 ✓ ⑤멤버 ETF→전체화면 ETF 상세 push(결정 A(b), 2-Pane·COMPACT 양쪽) ✓ ⑥**라이트/다크** 양쪽 가독(다크: 흰텍스트/다크surface, ETF수칩 amber, 배지 녹색, 흰-흰/검-검 없음) ✓ ⑦**2-Pane↔COMPACT**: wm size 1080×2340(443dp)로 COMPACT 강제 — 하단 nav 전환, 결정 B(a) 로컬토글 상세 교체+`BackHandler`(back=목록 복귀, 앱종료/탭전환 아님) ✓ ⑧**폴드 전환 상태보존**: COMPACT→wide 재구성 후 키워드 탭·쿼리 `rememberSaveable` 유지 ✓ ⑨빈뷰: 검색 비매칭("ZZZ")→"분류할 ETF가 없습니다 / ETF 목록 탭에서 수집" 렌더(ETF-0 문구 실증). **잔여 저severity nit 2건(비블로커)**: (a) 2-Pane 좁은 목록열에서 헤더 `Row(SpaceBetween)` "키워드 N개"+"마지막 갱신"이 폭을 꽉 채워 간격 0으로 붙음(COMPACT은 정상) — `KeywordListContent.kt:78~106`. (b) 쿼리 비매칭으로 groups 빈 경우도 `noKeywords=false` 분기라 "ETF 목록 탭에서 수집" 안내(실제 ETF는 존재·쿼리가 걸러낸 케이스) — `KeywordListContent.kt:108~114`. 스펙 미정의 엣지. **키워드 탭 Phase 0~4 전체 완료.**
- **Phase 1 완료(2026-07-21)**: `presentation/keyword/KeywordViewModel.kt` 신규(`@HiltViewModel`). `EtfViewModel`(prefs 로드)+`ThemeViewModel`(파생 StateFlow) 패턴 미러. `combine(etfRepository.getAllEtfs(), _includeKeywords, _query, _sortMode)` → `groupEtfsByKeyword` → `stateIn(WhileSubscribed(5_000), emptyList())`. 포함 키워드는 `init`에서 `loadEtfKeywordFilter(context).includeKeywords` 로드. 파생 `groupCount`/`lastUpdatedAt`. 기본 정렬 `ETF_COUNT`. `refresh()`=`WorkManagerHelper.runEtfUpdateNow`(기존 ETF 워커 재사용). `KeywordViewModelTest` 12건 신규(MockK static settings + turbine): 초기상태·includeKeywords 로드·그룹핑 방출·멤버0 제외·빈 키워드 빈목록·query 필터·NAME/AVG_RETURN 정렬·groupCount/lastUpdatedAt 파생·동적 source Flow 재emit. `refresh()`는 object `WorkManagerHelper` 의존이라 단위테스트 제외(ThemeViewModelTest 컨벤션, Phase 4 실기). `--tests "*KeywordViewModelTest"` BUILD SUCCESSFUL(tests=12/0 fail). DB/API/워커 변경 0. UI(Phase 2~3) 미착수.
